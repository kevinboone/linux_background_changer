...
extern KPath   *kpath_new_from_kstring (const KString* path);
...
/** Expands a KPath into a KList of KPaths, one for each entry in the
 * directory. See the KPE_XXX flags for setting the behaviour of
 * the expansion.
...
/** Get filename extension. If there isn't one, returns an empty string. */
