...
/** Split a KString into a list of KStrings, one each for each token. Tokens
 * are separted by any of the chars. These functions except the input string
 * to be non-null but, so long as that is the case, they will always return
 * a list. However, if the input string is empty, the list will be of zero
 * length */ 
extern KList        *kstring_tokenize (const KString *self, const KString *chars);
extern KList        *kstring_tokenize_utf8 (const KString *self, const UTF8 *chars);
extern KList        *kstring_tokenize_utf32 (const KString *self, const UTF32 *chars);
