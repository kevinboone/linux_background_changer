...
/*============================================================================
  ...
  ==========================================================================*/
KList *kstring_tokenize (const KString *self, const KString *chars)
  {
  KLOG_IN
  KList *ret = kstring_tokenize_utf32 (self, chars->str); 
  KLOG_OUT
  return ret;
  }
...
/*============================================================================
  ...
  ==========================================================================*/
KList *kstring_tokenize_utf8 (const KString *self, const UTF8 *chars)
  {
  KLOG_IN
  KString *temp = kstring_new_from_utf8 (chars);
  KList *ret = kstring_tokenize (self, temp);
  kstring_destroy (temp);
  KLOG_OUT
  return ret;
  }
...
/*============================================================================
  ...
  ==========================================================================*/
static const UTF32 *kstring_strchr_32 (const UTF32 *s, UTF32 c)
  {
  KLOG_IN
  const UTF32 *ret = NULL;
  int p = 0;
  //printf ("ststr=%S c=%c\n", s, (char)c);
  while (*s && !ret)
    {
    if (*s == c) ret = (const UTF32 *)(s + p);
    s++;
    p++;
    }
  KLOG_OUT
...
/*============================================================================
  ...
  ==========================================================================*/
KList *kstring_tokenize_utf32 (const KString *self, const UTF32 *chars)
  {
  KLOG_IN
  assert (self != NULL);
  assert (chars != NULL);

  KString *tok = kstring_new_empty();

  KList *list = klist_new_empty ((KListFreeFn)kstring_destroy); 
  if (self->length > 0)
    {
    for (int i = 0; i < self->length; i++)
      ...
      BOOL in_sep = FALSE;
      UTF32 c = self->str[i];
      if (kstring_strchr_32 (chars, c))
        is_sep = TRUE;
      else
        is_sep = FALSE;

      //printf ("i=%d char=%c sep=%d\n", i, (char)c, is_sep);

      if (!is_sep && !in_sep)
        {
        kstring_append_char (tok, c);
	}
	...
	if (tok->length > 0)
	  {
          klist_append (list, kstring_strdup (tok));
	  kstring_clear (tok);
	  }
	}
      else //is_sep && in_sel
        {
        // Do nothing - skip separators
	}
      }
    }
