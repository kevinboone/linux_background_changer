...
/*============================================================================
  ...
  kpath_new_from_kstring

  ==========================================================================*/
KPath *kpath_new_from_kstring (const KString* path) 
  {
  KLOG_IN
  KPath *ret = (KPath *)kstring_strdup (path);
  KLOG_OUT
  return ret;
  }
...
/*============================================================================
  ...
  ==========================================================================*/
KString *kpath_get_ext (const KPath *s)
  {
  KLOG_IN
  static UTF32 dot[] = {'.', 0};
  KString *ret = NULL;
  KString *filename = kpath_get_filename (s);
  int p = kstring_find_utf32 (filename, dot); 
  if (p >= 0)
    {
    ret = kstring_substring (filename, p + 1, 0);
    }
  else
    {
    ret = kstring_new_empty();
    }
  kstring_destroy (filename);
...
int kpath_open_write (const KPath *self)
  ...
  ret = open ((char *)path, O_WRONLY | O_CREAT | O_TRUNC, 0777);
  free (path);
  KLOG_OUT
  return ret;
  }
